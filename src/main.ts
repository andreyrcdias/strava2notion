...
async function filterNewActivities(activities: Activity[]): Promise<Activity[]> {
  const newActivities = await Promise.all(
    activities.map(async (activity) => {
      const exists = await existsAcitivity(NOTION_DATABASE_ID as string, activity.id);
...
async function fillNotionDatabase(): Promise<void> {
  try {
    ...
    console.log('Total new activities to add in Notion:', filteredActivities.length);
